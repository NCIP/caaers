...
public class TreatmentInformationBusinessRulesTest extends
	...
	public String getRuleFile() {
		return "treatment_information_rule.xml";
	}
	
	
	/**
	 * RuleName : SEC_BR5A_CHK
	Rule : Protocol Agents must be provided if Course Information has been provided.
	Error Code : SEC_BR5A_ERR
	Error Message : PROTOCOL_AGENTS must be provided if COURSE_INFORMATION is provided
	...
	public void testNoAgent_WithCourse() throws Exception {
		...
		assertSameErrorCount(errors, 1, "When Couse information is provided without Agent information");
	...
	public void testOnlyTACPresent() throws Exception {
		...
		assertEquals("There should not be any error, when there is only TAC",0, errors.getErrorCount());
	...
	public void testOnlyDescriptionPresent() throws Exception {
		...
		assertEquals("There should not be any error, when there is only Other Description or dose level",0, errors.getErrorCount());
	...
	public void testBothTAC_AndOtherDescriptionPresent() throws Exception {
		...
		assertEquals("There should be  error, when there is both TAC and Other Description or dose level",1, errors.getErrorCount());
	...
	public void testNoTAC_AndNoOtherDescriptionPresent() throws Exception {
		...
		assertEquals("There should not be any error, when there is no TAC and no Other Description or dose level",0, errors.getErrorCount());
	}
	...
	public void testNullTreatmentInformation() throws Exception {
		...
		assertEquals("There should not be any error, when there is no TreatmentInformation at all",0, errors.getErrorCount());
	}
	...
	public void testCourseNumberOfAE_Alone() throws Exception {
		...
		assertEquals("There should be error, when TotalNumberOfCourses is null" ,1, errors.getErrorCount());
	...
	public void testNoCourseAgent() throws Exception {
		...
		assertEquals("There should not be any error when there are no course agents", 0, errors.getErrorCount());
	}
	...
	public void testNoAdminDelay_And_NoModifiedDose() throws Exception {
		...
		assertEquals("There should not be any error when there are course agents, having no admin delay and modified dose",
	...
	public void testAdminDelay_WithModifiedDose() throws Exception {
		...
		assertEquals("There should not be any error when both Admin Delay and Modified dose", 0 , errors.getErrorCount());
	}
	...
	public void testModifiedDose_Alone() throws Exception {
		...
		assertTrue("There should be errors when there is only Modified Dose, with out Admin delay", errors.getErrorCount() > 0);
	...
	public void testModifiedDose_Alone_For_OneOutOfTwo() throws Exception {
		...
		assertEquals("Error code should be same", "PAG_BR1A_ERR", errors.getErrorAt(0).getCode());
		assertEquals("Error replacement value should be 1", 2, errors.getErrorAt(0).getReplacementVariables()[0]);
	...
	public void testAdminDelay_Alone() throws Exception {
		...
		assertTrue("There should be errors when there is only Admin Delay, with out Modified Dose", errors.getErrorCount() > 0);
		assertEquals("There should be 2 errors", 2, errors.getErrorCount());
	...
	*/
	public void testLastAdministeredDate_InvestigationalStudyAgents() throws Exception {
		...
		assertEquals("There should not be any error, when ind agents have last administred date", 0, errors.getErrorCount());		
	}
	...
	*/
	public void testNoLastAdministeredDate_InvestigationalStudyAgents() throws Exception {
		...
		assertEquals("Error replacement variable should be correct",2, errors.getErrorAt(1).getReplacementVariables()[0]);
	}
	...
	Error Message : LAST_ADMINISTERED_DATE must be entered for an investigational agent.
	*/
	public void testNoLastAdministeredDate_ForOneOutOfTwo_InvestigationalStudyAgents() throws Exception {
		ExpeditedAdverseEventReport aeReport = createAEReport();
		int i = 0;
		for(CourseAgent ca : aeReport.getTreatmentInformation().getCourseAgents()){
		...
		assertEquals("Error code should be same", "PAG_BR3_ERR", errors.getErrorAt(0).getCode());
		assertEquals("Error replacement variable should be correct", 2, errors.getErrorAt(0).getReplacementVariables()[0]);
