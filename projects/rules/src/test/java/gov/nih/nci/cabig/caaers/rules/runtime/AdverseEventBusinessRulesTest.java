...
public class AdverseEventBusinessRulesTest extends
	...
	public String getRuleFile() {
		// TODO Auto-generated method stub
		return "adverse_event_rule.xml";
	}
    
	/**
	 * RuleName : AER_BR3_CHK
	Logic : "'Hospitalization' must be provided if 'Grade' greater than 2"
	Error Code : AER_BR3_ERR
	Error Message : HOSPITALIZATION must be provided if GRADE is greater than or equal to 3
	 */
	...
	public void testGradeSEVERE_HospitalizationNONE() throws Exception{
		...
		ValidationErrors errors = fireRules(aeReport);
		assertCorrectErrorCode(errors, "AER_BR3_ERR", "When  grade is SEVERE and  hospitalization is NONE");
	...
	public void testGradeDEATH_HospitalizationPROLONGED() throws Exception{
		...
		ValidationErrors errors = fireRules(aeReport);
		assertNoErrors(errors, "When  grade is DEATH and  hospitalization is PROLONGED_HOSPITALIZATION");
	...
	public void testGradeSEVER_And_HospitalizationNONE_For_Second() throws Exception {
		...
		assertEquals("Correct replacement values", 2,errors.getErrorAt(0).getReplacementVariables()[0]);
	}
	...
	*/
	public void testNoStartDateForAllAEs() throws Exception {
		...
		assertSameErrorCount(errors, 1, "There should be error when Primary AE dont have start date");
	}
	...
	public void testWrongEndDateInOneOutOfTwoAEs() throws Exception {
		...
		assertCorrectErrorCode(errors, "AER_BR5_ERR");
		assertEquals("Correct replacement in error", 2, errors.getErrorAt(0).getReplacementVariables()[0]);
	...
	public void testDuplicateCTC() throws Exception {
		...
		assertSameErrorCount(errors, 1);
		assertEquals("Replacement variables should be same", 2 , errors.getErrorAt(0).getReplacementVariables()[0]);
