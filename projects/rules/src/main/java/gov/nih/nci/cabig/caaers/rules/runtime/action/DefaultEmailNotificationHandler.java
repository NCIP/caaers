...
public class DefaultEmailNotificationHandler implements NotificationHandler {
    ...
    public void performNotify(ActionContext actionContext, RuleContext ruleContext) {
        ...
        /**
         * 1. Where is the Email Service? 2. What is the Email Content? 3. 4. To whom all email
         * needs to be send? 5. How many times the email needs to be send?
         */
    ...
    public void testSendSMTPMAil(Integer actionId, RuleContext ruleContext) {
        ...
        NotificationDao notificationDao = (NotificationDao) applicationContext
                        .getBean("NotificationDao");
        ...
        try {
            if (emailInfo != null) emailService.send(emailInfo, smtpConfig);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
    ...
    private EmailInfo getFiveDayMail(RuleContext ruleContext) {
        ...
        if (adverseEventSDO != null) {
            ...
            content = content + "Adverse Event Hospitalization ==== "
                            + adverseEventSDO.getHospitalization() + "\n";
        }
    ...
    private EmailInfo getTenDayMail(RuleContext ruleContext) {
        ...
        String content = "Sample Content of the Email ... This is a test mail from Rule engine....The real content needs to be read from DB...\n ";
        if (adverseEventSDO != null) content = content + "Adverse Event Category ==== "
                        + adverseEventSDO.getCategory();
        emailInfo.setContent(content);
        return emailInfo;
    }
