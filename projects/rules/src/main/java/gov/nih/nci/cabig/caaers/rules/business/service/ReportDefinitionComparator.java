...
public class ReportDefinitionComparator implements Comparator<ReportDefinition>{
	...
	/**
	 * The logic for comparing these reports definitons is reverse in the
	 * sense that, lesser is the period of reporting, higher is the priority
	 * In other words, we are comparing the priorties
	 */
	...
	private int compareTimeScale(ReportDefinition o1, ReportDefinition o2){
		...
		if (o1.getTimeScaleUnitType().getCode().intValue()>o2.getTimeScaleUnitType().getCode().intValue()){
			  return -1;
			 }
		if (o1.getTimeScaleUnitType().getCode().intValue()<o2.getTimeScaleUnitType().getCode().intValue()){
			  return 1;
