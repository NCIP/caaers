...
public class LogObjectFormatter {
	...
	public String toTextTable(DecisionContext dc) throws Exception{
		...
		rest = headerWidth-(colWidth+2+dc.getFiredRuleSetInfo().getOrganizationName().length());
		builder.append(this.blank(rest-4));
		builder.append(columnSeperator());
		builder.append("\n");
		builder.append(offSet);
		...
		builder.append(columnSeperator());
		builder.append(this.blank(x-1));
		builder.append("Asserted Objects");
		rest = headerWidth-(x+k);
		builder.append(this.blank(rest-2));
		builder.append(columnSeperator());
		builder.append("\n");
		builder.append(offSet);
        ...
        */
		...
		for(int s=0;s<objects.size();s++){
			...
			if(obj instanceof Study){
				drawStudy(builder,offSet,headerWidth,(Study)obj);
			}
			if(obj instanceof StudyAgent){
