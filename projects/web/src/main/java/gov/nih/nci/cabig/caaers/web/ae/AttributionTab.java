...
public class AttributionTab extends AeTab {
    ...
    protected AttributionTab() {
        super("Attribution", ExpeditedReportSection.ATTRIBUTION_SECTION.getDisplayName(),
                        "ae/attribution");
    }
    ...
    private List<AttributionBlock> createBlocks(ExpeditedAdverseEventReport report) {
        ...
        blocks.add(new AttributionBlock("Disease", createGroups(
                        CauseAndAttributionAccessor.DISEASE, report)));
        blocks.add(new AttributionBlock("Course", "Course", createGroups(
                        CauseAndAttributionAccessor.COURSE_AGENT, report)));
        blocks.add(new AttributionBlock("Surgery intervention", createGroups(
                        CauseAndAttributionAccessor.SURGERY, report)));
        blocks.add(new AttributionBlock("Radiation intervention", createGroups(
                        CauseAndAttributionAccessor.RADIATION, report)));
        blocks.add(new AttributionBlock("Medical device", createGroups(
                        CauseAndAttributionAccessor.DEVICE, report)));
        blocks.add(new AttributionBlock("Concomitant medication", createGroups(
                        CauseAndAttributionAccessor.CONCOMITANT_MEDICATION, report)));
        blocks.add(new AttributionBlock("Other cause", createGroups(
                        CauseAndAttributionAccessor.OTHER_CAUSE, report)));
        return blocks;
    }
    ...
    private static <D extends DomainObject, A extends AdverseEventAttribution<D>> List<InputFieldGroup> createGroups(
