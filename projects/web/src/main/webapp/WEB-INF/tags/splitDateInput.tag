...
<table cellspacing="0" cellpadding="0" border="0" class="split-date-wrap">
    <tr>
        <td>
            <form:input path="${path}.monthString" cssClass="${empty monthValue && monthRequired ? 'required' : not empty monthValue ? 'valueOK' : ''} ${monthRequired ? 'required' : ''} ${required and monthRequired ? cssClass :'validate-NUMERIC' }" maxlength="2" size="2"/>/
        ...
        <td>
            <form:input path="${path}.yearString" cssClass="${empty yearValue && yearRequired ? 'required' : not empty yearValue ? 'valueOK' : ''} ${yearRequired ? 'required' : ''} ${cssValue} split-date ${required and yearRequired ? cssClass:'validate-NUMERIC'}" maxlength="4" size="4"/>
