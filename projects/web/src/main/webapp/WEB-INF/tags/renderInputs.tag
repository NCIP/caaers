...
<caaers:renderFilter elementID="${field.propertyName}">
    <c:choose>
        <c:when test="${field.categoryName == 'text'}"><form:input path="${field.propertyName}" disabled="${disabled}"
                                                                   size="${empty size ? field.attributes.size : size}"
                                                                   title="${field.displayName}"
                                                                   cssClass="${field.validatorClassName}"/></c:when>
        <c:when test="${field.categoryName == 'date'}"><tags:dateInput path="${field.propertyName}"
                                                                       title="${field.displayName}"
                                                                       cssClass="${field.validatorClassName}"/></c:when>
        <c:when test="${field.categoryName == 'split_date'}"><tags:splitDateInput
                ...
                required="${field.required}" path="${field.propertyName}"/></c:when>
        <c:when test="${field.categoryName == 'textarea'}"><form:textarea path="${field.propertyName}"
                                                                          disabled="${disabled}"
                                                                          cols="${not empty field.attributes.cols ? field.attributes.cols : ''}"
                                                                          rows="${not empty field.attributes.rows ? field.attributes.rows : ''}"
                                                                          title="${field.displayName}"
                                                                          cssClass="${field.validatorClassName}"/></c:when>
        <c:when test="${field.categoryName == 'checkbox'}"><form:checkbox id="${field.propertyName}"
                                                                          path="${field.propertyName}"
                                                                          disabled="${disabled}"
                                                                          cssClass="${cssClass}"/></c:when>
        <c:when test="${field.categoryName == 'inplace_text'}"><ui:inplaceTextField
                path="${field.propertyName}"/></c:when>
        <c:when test="${field.categoryName == 'label'}"><ui:value propertyName="${field.propertyName}"/></c:when>
        <c:when test="${field.categoryName == 'image'}"><img
                src="<c:url value="/images/chrome/spacer.gif" />"/></c:when>
        <c:when test="${field.categoryName == 'radio'}"><form:radiobutton path="${field.propertyName}"
                                                                          disabled="${disabled}"
                                                                          cssClass="${field.required ? 'validate-NOTEMPTY' : ''} ${cssClass}"
                                                                          value="${field.attributes.defaultValue}"/></c:when>
        <c:when test="${field.categoryName == 'select'}">
            <form:select path="${field.propertyName}" items="${field.attributes.options}" disabled="${disabled}"
                         title="${field.displayName}"
                         cssClass="${cssClass} ${field.required ? 'validate-NOTEMPTY' : ''}"/>
        </c:when>
        ...
        <c:when test="${field.categoryName == 'autocompleter'}">
            <input size="${empty size ? empty field.attributes.size ? '50' : field.attributes.size : size}" type="text"
                   ...
                   class="autocomplete ${cssClass} ${field.required ? 'validate-NOTEMPTY' : ''}"/>
            <tags:indicator id="${field.propertyName}-indicator"/>
            <c:if test="${field.attributes.enableClear and not disabled}"><input type="button"
    ...
    <c:if test="${not empty field.attributes.help}">
        <tags:hoverHelp path="${field.propertyName}"><spring:message code="${field.attributes.help}"
                                                                     text="No help available ${field.attributes.help}"/></tags:hoverHelp>
    </c:if>
