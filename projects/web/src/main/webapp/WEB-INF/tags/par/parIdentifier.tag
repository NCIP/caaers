...
<tr id="${sectionClass}-${listEditorIndex}" class="${sectionClass}" >
 <c:forEach items="${fieldGroups[mainGroup].fields}" var="field">
   ...
   <c:if test="${(fn:endsWith(field.propertyName, 'systemName')) &&  (identifier.class.name =='gov.nih.nci.cabig.caaers.domain.SystemAssignedIdentifier') }">
	<td>
		<tags:renderInputs field="${field}"/>
	</td>
   </c:if>
   <c:if test="${(fn:endsWith(field.propertyName, 'organization')) &&  (identifier.class.name =='gov.nih.nci.cabig.caaers.domain.OrganizationAssignedIdentifier') }">
	<td>
		<tags:renderInputs field="${field}" />
	</td>
   </c:if>
