					...
					<div class="${cIndex.index % 2 gt 0 ? 'odd' : 'even' } autoclear cmt" >
						...
						<c:choose>
							<c:when test="${comment.editable == true and fn:contains(comment.userId, command.userId)}">
								<chrome:division title='${comment.autoGeneratedText } on ${dt } <a href="javascript:setupEditComment(${comment.id})">(<img src="${editImage}" alt="edit" />edit)</a>' collapsable="true" id="cmt${cIndex.index}" collapsed="${cIndex.index lt noOfComments}">
