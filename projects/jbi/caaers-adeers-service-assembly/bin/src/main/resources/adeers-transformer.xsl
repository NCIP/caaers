...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="AdverseEventReport">
        <CAEERS_AEREPORT_ID><xsl:value-of select="id"/></CAEERS_AEREPORT_ID>
        <EXTERNAL_SYSTEMS><xsl:value-of select="EXTERNAL_SYSTEMS"/></EXTERNAL_SYSTEMS>
        ...
        <INSTITUTION_INFORMATION>
            <xsl:attribute name="CTEP_ID"><xsl:value-of select="StudyParticipantAssignment/StudySite/Organization/nciInstituteCode"/></xsl:attribute>
            <INSTITUTION_NAME><xsl:value-of select="StudyParticipantAssignment/StudySite/Organization/name"/></INSTITUTION_NAME>
        </INSTITUTION_INFORMATION>
        ...
        <TREATMENT_ASSIGNMENT_INFORMATION>
            ...
            <OTHER_TREATMENT_ASSIGNMENT><!-- SAMPL --></OTHER_TREATMENT_ASSIGNMENT>
        </TREATMENT_ASSIGNMENT_INFORMATION>
        <COURSE_INFORMATION>
        ...
        <DESCRIPTION_OF_EVENT>
            ...
            <!-- 
            <AUTOPSY_PERFORMED>SAMPL</AUTOPSY_PERFORMED>
            -->
        </DESCRIPTION_OF_EVENT>
        <PATIENT_INFORMATION>
            ...
            <xsl:if test="ParticipantHistory/baselinePerformanceStatus">
                <BASELINE_PERFORMANCE_STATUS>
                    <xsl:variable name="bps" select="ParticipantHistory/baselinePerformanceStatus"></xsl:variable>
        ...
        <xsl:for-each select="AdverseEventPriorTherapy">
            <PRIOR_THERAPY>
                ...
                <xsl:for-each select="PriorTherapyAgent">
                    <CHEMO_AGENT_NAME><xsl:value-of select="Agent/name"/></CHEMO_AGENT_NAME>
                </xsl:for-each>
            </PRIOR_THERAPY>
        </xsl:for-each>
        ...
        <PROTOCOL_AGENT>
            ...
            <xsl:variable name="totalDose" select="TreatmentInformation/CourseAgent/Dose/amount"></xsl:variable>
            
            <TOTAL_DOSE_ADMINISTERED><xsl:value-of select="TreatmentInformation/CourseAgent/Dose/amount"/></TOTAL_DOSE_ADMINISTERED>
            <LAST_ADMINISTERED_DATE>
                <xsl:call-template name="standard_date">
                    <xsl:with-param name="date" select="TreatmentInformation/CourseAgent/lastAdministeredDate"/>
                </xsl:call-template>
            </LAST_ADMINISTERED_DATE>
            <DOSE_UOM><xsl:value-of select="TreatmentInformation/CourseAgent/Dose/units"/></DOSE_UOM>
            <PROTOCOL_AGENT_COMMENTS/>
            <xsl:if test="TreatmentInformation/CourseAgent/ModifiedDose/amount">
                <xsl:variable name="adjusted" select="TreatmentInformation/CourseAgent/ModifiedDose/amount"></xsl:variable>
            ...
            <xsl:choose>
                <xsl:when test="TreatmentInformation/administrationDelayAmount"><AGENT_DELAYED>Yes</AGENT_DELAYED></xsl:when>
                <xsl:otherwise><AGENT_DELAYED>No</AGENT_DELAYED></xsl:otherwise>
            </xsl:choose>
            <xsl:if test="TreatmentInformation/administrationDelayAmount"><DELAY><xsl:value-of select="TreatmentInformation/administrationDelayAmount"/></DELAY></xsl:if>
            <xsl:if test="TreatmentInformation/administrationDelayUnits"><DELAY_UOM><xsl:value-of select="TreatmentInformation/administrationDelayUnits"/></DELAY_UOM></xsl:if>
        ...
        <xsl:for-each select="AdverseEvent">
            <ADVERSE_EVENT_CTC>
                ...
                <CATEGORY> <xsl:value-of select="AdverseEventCtcTerm/ctc-term/CtcCategory/name"/> </CATEGORY>
                <AE_TERM>
        ...
        <ADDITIONAL_INFORMATION>
            <xsl:if test="AdditionalInformation/autopsyReport = 'true'"><ADDITIONAL_INFO_NAME>Autopsy Report</ADDITIONAL_INFO_NAME></xsl:if>
            <xsl:if test="AdditionalInformation/consults = 'true'"><ADDITIONAL_INFO_NAME>Consults</ADDITIONAL_INFO_NAME></xsl:if>
            <xsl:if test="AdditionalInformation/dischargeSummary = 'true'"><ADDITIONAL_INFO_NAME>Discharge Summary</ADDITIONAL_INFO_NAME></xsl:if>
            <xsl:if test="AdditionalInformation/flowCharts = 'true'"><ADDITIONAL_INFO_NAME>Flow Sheets/Case Report Forms</ADDITIONAL_INFO_NAME></xsl:if>
            <xsl:if test="AdditionalInformation/labReports = 'true'"><ADDITIONAL_INFO_NAME>Laboratory Reports</ADDITIONAL_INFO_NAME></xsl:if>
            <xsl:if test="AdditionalInformation/obaForm = 'true'"><ADDITIONAL_INFO_NAME>OBA Form</ADDITIONAL_INFO_NAME></xsl:if>
            <xsl:if test="AdditionalInformation/other = 'true'"><ADDITIONAL_INFO_NAME>Other</ADDITIONAL_INFO_NAME></xsl:if>
            <xsl:if test="AdditionalInformation/pathologyReport = 'true'"><ADDITIONAL_INFO_NAME>Pathology Report</ADDITIONAL_INFO_NAME></xsl:if>
            <xsl:if test="AdditionalInformation/progressNotes = 'true'"><ADDITIONAL_INFO_NAME>Progress Notes</ADDITIONAL_INFO_NAME></xsl:if>
            <xsl:if test="AdditionalInformation/radiologyReports = 'true'"><ADDITIONAL_INFO_NAME>Radiology Reports</ADDITIONAL_INFO_NAME></xsl:if>
            <xsl:if test="AdditionalInformation/referralLetters = 'true'"><ADDITIONAL_INFO_NAME>Referral Letters</ADDITIONAL_INFO_NAME></xsl:if>
            <xsl:if test="AdditionalInformation/irbReport = 'true'"><ADDITIONAL_INFO_NAME>Summary Report Sent to IRB</ADDITIONAL_INFO_NAME></xsl:if>
            
            <xsl:if test="AdditionalInformation/otherInformation = 'true'"><ADDITIONAL_INFO_OTHER><xsl:value-of select="AdditionalInformation/otherInformation"/></ADDITIONAL_INFO_OTHER></xsl:if>
        </ADDITIONAL_INFORMATION>
    </xsl:template>
