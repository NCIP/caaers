...
//		setComplete();
		ApplicationContext applicationContext = new ClassPathXmlApplicationContext(
				new String[] {
						"classpath:/applicationContext-acls-dao-test.xml"});
		MutableAclService aclService = (MutableAclService) applicationContext
				.getBean("aclService");

		Authentication auth = new TestingAuthenticationToken("joshua",
				"ignored", new GrantedAuthority[] { new GrantedAuthorityImpl(
						"ROLE_ADMINISTRATOR") });
		auth.setAuthenticated(true);
		SecurityContextHolder.getContext().setAuthentication(auth);

		ObjectIdentity topParentOid = new ObjectIdentityImpl("gov.nih.nci.security.acegi.acls.test.SomeBean",
				new Long(100));
		ObjectIdentity middleParentOid = new ObjectIdentityImpl(
				"gov.nih.nci.security.acegi.acls.test.SomeBean", new Long(101));
		ObjectIdentity childOid = new ObjectIdentityImpl("gov.nih.nci.security.acegi.acls.test.SomeBean",
				new Long(102));
		...
		topParent.insertAce(null, BasePermission.READ, new PrincipalSid(auth),
				true);
		topParent.insertAce(null, BasePermission.WRITE, new PrincipalSid(auth),
				false);
		middleParent.insertAce(null, BasePermission.DELETE, new PrincipalSid(
				auth), true);
		child.insertAce(null, BasePermission.DELETE, new PrincipalSid(auth),
				false);
		...
		Map map = aclService.readAclsById(new ObjectIdentity[] { topParentOid,
				middleParentOid, childOid });
		...
		assertEquals(topParentOid, middleParent.getParentAcl()
				.getObjectIdentity());
		...
		try {
			child.isGranted(new Permission[] { BasePermission.READ },
					new Sid[] { new PrincipalSid(auth) }, true);
			fail("Should have thrown NotFoundException");
		} catch (NotFoundException expected) {
			assertTrue(true);
		...
		child.insertAce(null, BasePermission.CREATE, new PrincipalSid(auth),
				true);
		...
		assertEquals(BasePermission.DELETE.getMask(), entry.getPermission()
				.getMask());
