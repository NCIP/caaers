 ...
 * 
 */
public class LabViewerConsumerImpl extends LabViewerConsumerImplBase {
	...
	private static final String LABVIEWER_CONSUMER_BEAN_NAME = "labViewerConsumerBeanName";

	private static final String DEFAULT_LABVIEWER_CONSUMER_BEAN_NAME = "labViewerConsumer";
	
	private LabViewerConsumerI consumer;
	    
	public LabViewerConsumerImpl() throws RemoteException {
		super();
        String exp = ContainerConfig.getConfig().getOption(SPRING_CLASSPATH_EXPRESSION,DEFAULT_SPRING_CLASSPATH_EXPRESSION);
        String bean = ContainerConfig.getConfig().getOption(LABVIEWER_CONSUMER_BEAN_NAME,DEFAULT_LABVIEWER_CONSUMER_BEAN_NAME);
        ApplicationContext ctx = new ClassPathXmlApplicationContext(exp);
        consumer = (LabViewerConsumerI) ctx.getBean(bean);
	}
