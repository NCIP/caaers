...
import gov.nih.nci.ccts.grid.service.LabViewerConsumerConfiguration;
import java.io.File;
 ...
 * 
 */
public abstract class LabViewerConsumerResourceBase extends ReflectionResource implements Resource
                                                  {

	static final Log logger = LogFactory.getLog(LabViewerConsumerResourceBase.class);

	private LabViewerConsumerResourceConfiguration configuration;
    ...
    public LabViewerConsumerResourceBase() {
    }
	...
	public gov.nih.nci.cagrid.metadata.ServiceMetadata getServiceMetadata(){
		return ((LabViewerConsumerResourceProperties) getResourceBean()).getServiceMetadata();
	}
	
	public void setServiceMetadata(gov.nih.nci.cagrid.metadata.ServiceMetadata serviceMetadata ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(LabViewerConsumerConstants.SERVICEMETADATA);
	...
	public LabViewerConsumerResourceConfiguration getConfiguration() {
		...
		try {
			...
			this.configuration = (LabViewerConsumerResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
	...
	private void loadServiceMetadataFromFile() {
      if(getServiceMetadata()==null){
		try {
			...
			((LabViewerConsumerResourceProperties) this.getResourceBean()).setServiceMetadata((gov.nih.nci.cagrid.metadata.ServiceMetadata) Utils.deserializeDocument(dataFile.getAbsolutePath(),
