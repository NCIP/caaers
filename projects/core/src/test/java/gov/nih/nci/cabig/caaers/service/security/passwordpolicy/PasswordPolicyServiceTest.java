...
public class PasswordPolicyServiceTest extends CaaersTestCase {
	...
	expect(userService.userHasPassword(eq(userName), not(eq("real_password")))).andReturn(false).anyTimes();
	expect(userService.userHadPassword(userName, "0ld_Password")).andReturn(true).anyTimes();
	expect(userService.userHadPassword(eq(userName), not(eq("0ld_Password")))).andReturn(false).anyTimes();
