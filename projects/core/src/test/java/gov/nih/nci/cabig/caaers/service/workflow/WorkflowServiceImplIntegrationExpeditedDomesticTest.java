...
public class WorkflowServiceImplIntegrationExpeditedDomesticTest extends CaaersDbTestCase {
	...
	public void testNextTransitions() {
		...
		interruptSession();
		{
			...
			assertEquals("Submit For Physician Review", nextTransitions.get(0).getName());
			assertEquals("Physician Review", nextTransitions.get(0).getTo().getName());
		}
	}
	...
	public void testAdvanceWorkflow_ToApproveReportByPhysician(){
		...
		interruptSession();
		{
			...
			assertFalse(nextTransitions.isEmpty());
			assertEquals(1, nextTransitions.size());
			assertEquals("Submit to Central Office SAE Coordinator", nextTransitions.get(0).getName());
			assertEquals("Central Office SAE Coordinator Review", nextTransitions.get(0).getTo().getName());
	...
	public void testAdvanceWorkflow_ToCentralOfficeSAECoordinatorReview(){
		...
		interruptSession();
		{
			...
			assertEquals(2, nextTransitions.size());
			assertEquals("Approve Report", nextTransitions.get(1).getName());
			assertEquals("Submit Report To Sponsor", nextTransitions.get(1).getTo().getName());
	...
	public void testAdvanceWorkflow_ToSubmitToSponsor(){
		...
		interruptSession();
		{
			...
			assertEquals("Submit Report to Sponsor", nextTransitions.get(0).getName());
			assertEquals("End Expedited Domestic Flow", nextTransitions.get(0).getTo().getName());
		}
	}
	...
	public void testAdvanceWorkflow_ToSubmittedToSponsor(){
		...
		
		{
			...
			status = wfService.advanceWorkflow(id, "Submit Report to Sponsor");
			assertEquals(ReviewStatus.SUBMITTED_TO_SPONSOR, status);
		}
		interruptSession();
		{
			List<Transition> nextTransitions = wfService.nextTransitions(id, loginId);
