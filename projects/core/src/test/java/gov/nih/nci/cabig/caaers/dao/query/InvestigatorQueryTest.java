...
public class InvestigatorQueryTest extends TestCase {
	...
	public void testQueryConstructor() throws Exception {
		...
		assertEquals("wrong parsing for constructor",
				"SELECT distinct i from Investigator i left join fetch i.siteInvestigatorsInternal order by i.id",
	...
	public void testFilterByName() throws Exception {
		...
		assertEquals(
				"SELECT distinct i from Investigator i left join fetch i.siteInvestigatorsInternal WHERE lower(i.firstName) LIKE :firstName order by i.id",
				investigatorQuery.getQueryString());
		...
		assertEquals(
				"SELECT distinct i from Investigator i left join fetch i.siteInvestigatorsInternal WHERE lower(i.firstName) LIKE :firstName AND "
						+ "lower(i.lastName) LIKE :lastName order by i.id", investigatorQuery.getQueryString());
	...
	public void testFilterByIdentifier() throws Exception {
		...
		assertEquals(
				"SELECT distinct i from Investigator i left join fetch i.siteInvestigatorsInternal WHERE lower(i.nciIdentifier) LIKE :nciIdentifier order by i.id",
				investigatorQuery.getQueryString());
		assertEquals("wrong number of parameters", investigatorQuery.getParameterMap().size(), 1);
		assertTrue("missing paramenter name", investigatorQuery.getParameterMap().containsKey("nciIdentifier"));
		assertEquals("wrong parameter value", investigatorQuery.getParameterMap().get("nciIdentifier"), "%a%");
