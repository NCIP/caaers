...
public class ResearchStaffQueryTest extends TestCase {
    ...
    public void testFilterByTitle() throws Exception {
        ...
        assertTrue("missing paramenter name", reserachStaffQuery.getParameterMap().containsKey(
                        "name"));
        assertEquals("wrong parameter value", reserachStaffQuery.getParameterMap().get("name"),
                        "%a%");

        // reserachStaffQuery.filterByNciInstituteCode("b");
        // assertEquals("wrong number of parameters", reserachStaffQuery.getParameterMap().size(),
        // 2);
        // assertTrue("missing paramenter name",
        // reserachStaffQuery.getParameterMap().containsKey("nciInstituteCode"));
        // assertEquals("wrong parameter value",
        // reserachStaffQuery.getParameterMap().get("nciInstituteCode"), "%b%");
    ...
    public void testFilterByNCICode() throws Exception {
        ...
        // assertEquals(
        // "SELECT distinct o from ReserchStaff o WHERE o.nciInstituteCode LIKE :nciInstituteCode
        // order by o.id",
        // reserachStaffQuery.getQueryString());
        // assertEquals("wrong number of parameters", reserachStaffQuery.getParameterMap().size(),
        // 1);
        ...
        // reserachStaffQuery.getParameterMap().containsKey("nciInstituteCode"));
        // assertEquals("wrong parameter value",
        // reserachStaffQuery.getParameterMap().get("nciInstituteCode"), "%a%");
