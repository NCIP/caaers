 ...
 * 
 * @author sakkala
 * Generate a data report for a given query
 *    Objects Involved
...
//    		 Create FileReader Object
            ...
            outputStream.println("+---------- Auto generated report based on data retrieved from the caAERS database. ----------+");
            outputStream.println("");
            outputStream.println("");
            outputStream.println("+---------- INPUT QUERY. ----------+");
            outputStream.println("Get all Particpant whose subname (first name or last name) is 'Jack'");
            outputStream.println("");        
            outputStream.println(""); 
            outputStream.println("+------------Objects Involed . -------+");
            outputStream.println("Participant , Study, StudySite, StudyParticipantAssignment, ExpeditedAdverseEventReport , AdverseEventResponseDescription, Physician, ContactMechanism, Reporter, ");        
            outputStream.println("DiseaseHistory, CtepStudyDisease, DiseaseTerm, AnatomicSite, ParticipantHistory, RadiationIntervention, SurgeryIntervention, MedicalDevice, ConcomitantMedication");
            outputStream.println("AdverseEvent, ConcomitantMedicationAttribution, OtherCauseAttribution, CourseAgentAttribution, CourseAgent, Dose, StudyAgent, DiseaseAttribution, DiseaseHistory");
            outputStream.println("SurgeryAttribution, RadiationAttribution, DeviceAttribution, AdverseEventCtcTerm, Lab, SAEReportPriorTherapy, TreatmentInformation, AdverseEventCourse, CourseAgent, SAEReportPreExistingCondition"); 
            outputStream.println("");   
            outputStream.println("");             
            ...
            for (int j=0;j<participantList.size();j++) {
	            ...
	            outputStream.println("");
	            outputStream.println("");
	            outputStream.println("--- Retrieving the Study details for the selected participant. ---");
	            outputStream.println("--- Participant has "+participant.getStudies().size()+" Studies. ---");
	            outputStream.println("");
	            ...
	            for(int i=0;i<studies.size();i++){
	            	...
	            	outputStream.println("Study Terminology Version: "+study.getCtcVersion().getName()); 
	            	outputStream.println("Primary Sponsor: "+study.getPrimaryFundingSponsor().getOrganization().getName()); 
	            }
	            
	            outputStream.println("");
                outputStream.println("");
                outputStream.println("--- Retrieving the Expedited Adverse Event Report details for the selected participant. ---");
                ...
                for(int i=0;i<assignments.size();i++){
                	...
                	outputStream.println("--- Participant has "+spa.getAeReports().size()+" Expedited Reports. ---");
                    outputStream.println(""); 
