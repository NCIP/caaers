...
public class ParticipantAjaxableDomainObjectQueryTest extends TestCase {
	...
	private String baseQuery = "SELECT participant.id, participant.firstName, participant.lastName, participant.gender,participant.race,participant.ethnicity," +
            "identifier.value,identifier.primaryIndicator,spa.studySubjectIdentifier, studyIdentifier.value, spa.id, study.id FROM Participant participant " +
            "left join participant.identifiers as identifier left join participant.assignments as spa " +
            "left join spa.studySite as studySite left join studySite.study as study " +
            "left join study.identifiers as studyIdentifier order by participant.firstName " ;
    ...
    public void testFilterByStudyId() throws Exception {
        ...
        String qry = "SELECT participant.id, participant.firstName, participant.lastName, participant.gender,participant.race,participant.ethnicity,identifier.value,identifier.primaryIndicator,spa.studySubjectIdentifier, studyIdentifier.value, spa.id, study.id FROM Participant participant left join participant.identifiers as identifier left join participant.assignments as spa left join spa.studySite as studySite left join studySite.study as study left join study.identifiers as studyIdentifier join spa.studySite as ss join ss.study as study WHERE study.id =:studyId  order by participant.firstName";
    ...
    public void testParticipantsWithMatchingText() throws Exception {
        ...
        String qry = "SELECT participant.id, participant.firstName, participant.lastName, participant.gender,participant.race,participant.ethnicity,identifier.value,identifier.primaryIndicator,spa.studySubjectIdentifier, studyIdentifier.value, spa.id, study.id FROM Participant participant left join participant.identifiers as identifier left join participant.assignments as spa left join spa.studySite as studySite left join studySite.study as study left join study.identifiers as studyIdentifier WHERE (lower(participant.firstName) LIKE :firstName or lower(participant.lastName) LIKE :lastName or lower(identifier.value) LIKE :identifierValue or lower(spa.studySubjectIdentifier) LIKE :studySubjectIdentifier)  order by participant.firstName";
    ...
    public void testParticipantsWithMatchingTextTwoTexts() throws Exception {
        ...
        String qry ="SELECT participant.id, participant.firstName, participant.lastName, participant.gender,participant.race,participant.ethnicity,identifier.value,identifier.primaryIndicator,spa.studySubjectIdentifier, studyIdentifier.value, spa.id, study.id FROM Participant participant left join participant.identifiers as identifier left join participant.assignments as spa left join spa.studySite as studySite left join studySite.study as study left join study.identifiers as studyIdentifier WHERE ((lower(participant.firstName) LIKE :firstName AND lower(participant.lastName) LIKE :lastName) OR (lower(participant.lastName) LIKE :firstName AND lower(participant.firstName) LIKE :lastName))  order by participant.firstName";
        assertEquals(qry.trim(), query.getQueryString());
    ...
    public void testFilterByBothParticipantAndText() throws Exception {
       ...
       // String qry = "Select participant.id,participant.firstName,participant.lastName, participant.gender,participant.race,participant.ethnicity, identifier.value, identifier.primaryIndicator, study.shortTitle as st, study.id as studyId, sIdentifier.value, sIdentifier.primaryIndicator, studyOrgs.organization.name, studyOrgs.id, studyOrgs.class, studyOrgs.organization.nciInstituteCode, siteResearchStaff.researchStaff.id, ss.organization.id as assignedSiteId, ss.organization.name as assignedSite, ss.organization.nciInstituteCode as assignedSiteCode, spa.studySubjectIdentifier from Participant participant left join participant.identifiers as identifier left join participant.assignments as spa join spa.studySite as ss join ss.study as study join study.identifiers as sIdentifier join study.studyOrganizations as studyOrgs left join studyOrgs.studyPersonnelsInternal as stper left join stper.siteResearchStaff as siteResearchStaff WHERE (lower(participant.firstName) LIKE :firstName or lower(participant.lastName) LIKE :lastName or lower(identifier.value) LIKE :identifierValue or lower(spa.studySubjectIdentifier) LIKE :studySubjectIdentifier) AND study.id =:studyId order by participant.firstName";
        ...
        String qry = "SELECT participant.id, participant.firstName, participant.lastName, participant.gender,participant.race,participant.ethnicity,identifier.value,identifier.primaryIndicator,spa.studySubjectIdentifier, studyIdentifier.value, spa.id, study.id FROM Participant participant left join participant.identifiers as identifier left join participant.assignments as spa left join spa.studySite as studySite left join studySite.study as study left join study.identifiers as studyIdentifier join spa.studySite as ss join ss.study as study WHERE (lower(participant.firstName) LIKE :firstName or lower(participant.lastName) LIKE :lastName or lower(identifier.value) LIKE :identifierValue or lower(spa.studySubjectIdentifier) LIKE :studySubjectIdentifier) AND study.id =:studyId  order by participant.firstName";
    ...
    public void testFilterByPrimaryIdentifiers(){
    	...
    	String qry =  "SELECT participant.id, participant.firstName, participant.lastName, participant.gender,participant.race,participant.ethnicity,identifier.value,identifier.primaryIndicator,spa.studySubjectIdentifier, studyIdentifier.value, spa.id, study.id FROM Participant participant left join participant.identifiers as identifier left join participant.assignments as spa left join spa.studySite as studySite left join studySite.study as study left join study.identifiers as studyIdentifier WHERE identifier.primaryIndicator is true and sIdentifier.primaryIndicator is true  order by participant.firstName";
    	assertEquals(qry.trim(),query.getQueryString());
    }
 /*   
    ...
    public void testFilterParticipants(){
    	...
    	String qry = "SELECT participant.id, participant.firstName, participant.lastName, participant.gender,participant.race,participant.ethnicity,identifier.value,identifier.primaryIndicator,spa.studySubjectIdentifier, studyIdentifier.value, spa.id, study.id FROM Participant participant left join participant.identifiers as identifier left join participant.assignments as spa left join spa.studySite as studySite left join studySite.study as study left join study.identifiers as studyIdentifier WHERE (lower(identifier.value) LIKE :identifierValue or lower(spa.studySubjectIdentifier) LIKE :identifierValue) AND lower(participant.firstName) LIKE :firstName AND lower(participant.lastName) LIKE :lastName  order by participant.firstName";
