...
public class StudyHavingStudySiteQueryTest extends TestCase {
    ...
    public void testQueryConstructor() throws Exception {
        ...
        assertEquals("wrong parsing for constructor", "select distinct s from Study s", query.getQueryString());
    }

    public void testFilterByStudySiteName() throws Exception {
        StudyHavingStudySiteQuery query = new StudyHavingStudySiteQuery();
        query.filterByStudySiteName("a");
        assertEquals("select distinct s from Study s WHERE lower(ss.organization.name) LIKE :organizationName", query.getQueryString());
        assertEquals("wrong number of parameters", query.getParameterMap().size(), 1);
        assertTrue("missing paramenter name", query.getParameterMap().containsKey("organizationName"));
        assertEquals("wrong parameter value", query.getParameterMap().get("organizationName"), "%a%");

        query.filterByStudyShortTile("b");
        assertEquals("select distinct s from Study s WHERE lower(ss.organization.name) LIKE :organizationName AND lower(s.shortTitle) LIKE :shortTitle", query.getQueryString());
    ...
    public void testFilterByIdentifier() throws Exception {
        ...
        assertEquals("select distinct s from Study s join s.identifiers as identifier WHERE lower(identifier.value) LIKE :identifier", query.getQueryString());
        assertEquals("wrong number of parameters", query.getParameterMap().size(), 1);
        ...
        assertEquals("select distinct s from Study s WHERE lower(s.identifiers.value) LIKE :identifier", query.getQueryString());
        assertEquals("wrong number of parameters", query.getParameterMap().size(), 1);
        assertTrue("missing paramenter name", query.getParameterMap().containsKey("identifier"));
        assertEquals("wrong parameter value", query.getParameterMap().get("identifier"), "a");
    }
