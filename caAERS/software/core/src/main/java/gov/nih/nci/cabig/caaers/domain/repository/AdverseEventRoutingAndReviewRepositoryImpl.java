...
public class AdverseEventRoutingAndReviewRepositoryImpl implements AdverseEventRoutingAndReviewRepository {
	...
	public Long enactReportWorkflow(Report report) {
        ...
        variables.put(WorkflowService.VAR_WF_STUDY_NAME, studySite.getStudy().getDisplayName());
        variables.put(WorkflowService.VAR_WF_SUBJECT_NAME, assignment.getParticipant().getFullName());
        variables.put(WorkflowService.VAR_WF_COURSE_NAME, report.getAeReport().getReportingPeriod().getName());
    ...
    public List<TaskNotificationDTO> getTaskNotificationByUserLogin(String userLogin) {
        ...
        Collections.sort(dtos, new Comparator<TaskNotificationDTO>() {
        	public int compare(TaskNotificationDTO o1, TaskNotificationDTO o2) {
