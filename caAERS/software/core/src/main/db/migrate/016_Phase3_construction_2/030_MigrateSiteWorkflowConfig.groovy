	...
	void up(){
		...
		if (databaseMatches('postgresql')){
			...
			execute("insert into study_site_wf_cfgs (id, study_organization_id, workflow_cfg_id, name) " + 
                    "select seq_study_site_wf_cfgs_id.nextval,so.id ,(select id from workflow_configuration where workflow_definition_name = 'reportingperiod_coordinating_center'), " +
                    ...
                    "UNION " + 
                    "select seq_study_site_wf_cfgs_id.nextval,so.id ,(select id from workflow_configuration where workflow_definition_name = 'expedited_domestic'), " +
