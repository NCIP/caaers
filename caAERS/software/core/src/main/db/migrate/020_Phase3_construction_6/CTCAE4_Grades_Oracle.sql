...
WHERE grade_code = 1 AND term_id IN (
    ...
    WHERE c.version_id = 4 AND t.other_required = 1 AND grade_code = 1
);
...
WHERE grade_code = 2 AND term_id IN (
    ...
    JOIN ctc_terms t ON t.category_id = c.id
    JOIN ctc_grades g ON t.id = g.term_id
    WHERE c.version_id = 4 AND t.other_required = 1 AND grade_code = 2
);
...
WHERE grade_code = 3 AND term_id IN (
    ...
    JOIN ctc_terms t ON t.category_id = c.id
    JOIN ctc_grades g ON t.id = g.term_id
    WHERE c.version_id = 4 AND t.other_required = 1 AND grade_code = 3
);
...
WHERE grade_code = 4 AND term_id IN (
    ...
    JOIN ctc_terms t ON t.category_id = c.id
    JOIN ctc_grades g ON t.id = g.term_id
    WHERE c.version_id = 4 AND t.other_required = 1 AND grade_code = 4
);
...
WHERE grade_code = 5 AND term_id IN (
    ...
    JOIN ctc_terms t ON t.category_id = c.id
    JOIN ctc_grades g ON t.id = g.term_id
    WHERE c.version_id = 4 AND t.other_required = 1 AND grade_code = 5
);
