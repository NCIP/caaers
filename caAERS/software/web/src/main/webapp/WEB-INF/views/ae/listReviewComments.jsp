					...
					<div class="${cIndex.index % 2 gt 0 ? 'odd' : 'even' } autoclear cmt" >
						...
						<c:choose>
							<c:when test="${comment.editable == true and fn:contains(comment.userId, command.userId)}">
								<chrome:division title='${comment.autoGeneratedText } on ${dt } <a href="javascript:setupEditComment(${comment.id}, ${command.entityId })"><img src="${editImage}" alt="edit" height="16" title="Edit Comment..."/></a>
									<a href="javascript:deleteComment(${comment.id}, ${command.entityId })"  alt="delete"><img src="${deleteImage}" title="Delete Comment."/></a>'
