        ...
        <div id="taskbar">
            <c:if test="${not empty currentSection.tasks}">
				...
				<c:set var="lengthOfTask" value="${fn:length(task.displayName)}" />
                    <csmauthz:accesscontrol domainObject="${task}" authorizationCheckName="taskAuthorizationCheck">
                    	 <li class="${noOfTasks gt 4 ? 'gt4' : 'lte4'}"><a class="${(task == currentTask) || (task.displayName == currentTask.displayName) ?  'selected' : '' } ${(lengthOfTask gt 21 ? 'gt18' : '')}" id="secondlevelnav_${task.linkName}"  href="<c:url value="${task.url}"/>">
                    	 	<img class="${(lengthOfTask gt 21 ? 'imagegt18' : '')}" src="<c:url value="/images/blue/icons/${task.linkName}_icon.png" />?${requestScope.webCacheId}" alt=""/>
                    	 	<span class="spangt18"><c:out value="${task.displayName}" escapeXml="true"/></span></a>
                    	 </li>
                    </csmauthz:accesscontrol>
                </c:forEach>
				</ul>
