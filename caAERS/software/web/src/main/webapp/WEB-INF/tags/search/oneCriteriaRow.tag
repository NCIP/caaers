...
<c:if test="${criteriaParameter == null}">
	<tr id="criteria-${index }">
		...
		<td align="center" id="operator-td-${index }">
			<select style="width:200px;" id="operator-<c:out value="${index}" escapeXml="true"/>" onChange="javascript:updateOperator(${index });"
		...
		<td align="center" id="delete-td-${index }">
			<img src="<c:url value="/images/checkno.gif" />?${requestScope.webCacheId}" id="delete-<c:out value="${index}" escapeXml="true"/>" onClick="javascript:deleteCriteria(${index });"/>
		</td>
	</tr>
</c:if>
<c:if test="${criteriaParameter != null}">
