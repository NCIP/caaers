        ...
        <c:if test="${eachRow[3]}">
            <c:if test="${not empty command.study.otherMeddra}">
                ...
                <c:if test="${!displayOnly}">
                    <tags:autocompleter displayName="abcd" propertyName="otherMeddra-${eachRow[0]}" size="30"
                                        initialDisplayValue="${initialDisplayValue}"
                                        initialValue="${eachRow[2] == null ? '' : eachRow[2].id}"/>
                    <script>
                        AE.createStandardAutocompleter('otherMeddra-${eachRow[0]}',
                                function(autocompleter, text) {
                                    createAE.matchLowLevelTermsByCode(${terminologyVersionId}, text, function(values) { autocompleter.setChoices(values) })
                                },
            ...
            <c:if test="${empty command.study.otherMeddra}">
                <br>
                <c:if test="${displayOnly}">
                    <caaers:message code="LBL_aeReport.adverseEvents.detailsForOther" />&nbsp;${eachRow[4]}
                </c:if>
                <c:if test="${!displayOnly}">
                    <tags:requiredIndicator/>&nbsp;<caaers:message code="LBL_aeReport.adverseEvents.detailsForOther" />&nbsp;<input type="text" size="30" name="verbatim-${eachRow[0]}" value="${eachRow[4]}">
                </c:if>
            </c:if>
        </c:if>
    </td>
    <c:forEach varStatus="status" var="eachCheckBox" items="${eachRow}" begin="5">
        <td class="col-epoch-${status.index-5}"><input  ${displayOnly?'disabled':''}
                class="ck${status.index-5} ck-${eachRow[0]}" id="ck${status.index-5}-${eachRow[0]}"
                name="epoch[${status.index-5}]" value="${eachRow[0]}"
                type="checkbox" ${(eachRow[status.index] == true)?'checked':''}/></td>
    </c:forEach>
    <c:if test="${!displayOnly}">
