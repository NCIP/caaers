package gov.nih.nci.ess.sr.service.service.globus.resource;
...
import gov.nih.nci.ess.sr.service.common.SafetyReportingConstants;
import gov.nih.nci.ess.sr.service.stubs.SafetyReportingResourceProperties;
 ...
 * 
 */
public abstract class SafetyReportingResourceBase extends ReflectionResource implements Resource
                                                  {

	static final Log logger = LogFactory.getLog(SafetyReportingResourceBase.class);

	private SafetyReportingResourceConfiguration configuration;
    ...
    public SafetyReportingResourceBase() {
    }
	...
	public gov.nih.nci.cagrid.metadata.ServiceMetadata getServiceMetadata(){
		return ((SafetyReportingResourceProperties) getResourceBean()).getServiceMetadata();
	}
	
	public void setServiceMetadata(gov.nih.nci.cagrid.metadata.ServiceMetadata serviceMetadata ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(SafetyReportingConstants.SERVICEMETADATA);
	...
	public SafetyReportingResourceConfiguration getConfiguration() {
		...
		try {
			...
			this.configuration = (SafetyReportingResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
	...
	private void loadServiceMetadataFromFile() {
      if(getServiceMetadata()==null){
		try {
			...
			((SafetyReportingResourceProperties) this.getResourceBean()).setServiceMetadata((gov.nih.nci.cagrid.metadata.ServiceMetadata) Utils.deserializeDocument(dataFile.getAbsolutePath(),
